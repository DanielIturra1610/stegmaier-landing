...
const CertificatesPage: React.FC = () => {
  ...
  // Estado para filtros
  const [filterStatus, setFilterStatus] = useState<'all' | 'available' | 'earned'>('all');
  ...
  const getFilteredContent = () => {
    if (filterStatus === 'earned') {
      ...
      return { certificates: certificates, enrollments: enrollments };
    }
  };

  const { certificates: filteredCertificates, enrollments: availableEnrollments } = getFilteredContent();
    ...
    <div className="space-y-6 pb-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          ...
          <div className="mt-4 md:mt-0">
            <select
              ...
              value={filterStatus}
              onChange={(e) => setFilterStatus(e.target.value as 'all' | 'available' | 'earned')}
            >
              <option value="all">Todos los cursos</option>
              <option value="earned">Certificados obtenidos</option>
              <option value="available">Disponibles para certificar</option>
            </select>
          </div>
        </div>
      </header>
      ...
      {loading ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="mt-4 text-gray-500">Cargando certificados...</p>
        </div>
      ) : error ? (
        <div className="bg-red-50 border border-red-200 rounded-xl p-6">
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-red-800">Error al cargar certificados</h3>
        ...
        <>
          ...
          {filteredCertificates.length > 0 || (filterStatus === 'available' && availableEnrollments.length > 0) ? (
            ...
            {filteredCertificates.map((certificate) => (
              ...
              {/* Mostrar enrollments disponibles para certificar si el filtro es 'available' */}
              {filterStatus === 'available' && availableEnrollments.length > 0 && (
                <div>
                  <h3 className="text-lg font-medium text-gray-900 mb-4">Cursos completados sin certificado</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {availableEnrollments.map((enrollment: any) => (
                      <div key={enrollment.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                        <h4 className="font-medium text-gray-900 mb-2">{enrollment.course?.title || 'Curso'}</h4>
                        <p className="text-sm text-green-600 mb-4">✓ Curso completado al 100%</p>
