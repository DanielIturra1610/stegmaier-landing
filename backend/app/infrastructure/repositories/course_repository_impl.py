...
class MongoDBCourseRepository(CourseRepository):
    ...
    async def create(self, course: Course) -> Course:
        ...
        result = await self.db[self.collection_name].insert_one(course_dict)
        course.id = str(result.inserted_id)
        return course
    
    async def get_by_id(self, course_id: str) -> Optional[Course]:
        """Obtener curso por ID"""
        course_data = await self.db[self.collection_name].find_one({"_id": ObjectId(course_id)})
        if course_data:
    ...
    async def list(self, skip: int = 0, limit: int = 100, **filters) -> List[Course]:
        ...
        courses_data = await self.db[self.collection_name].find(query).skip(skip).limit(limit).to_list(length=limit)
    ...
    async def get_by_instructor(self, instructor_id: str) -> List[Course]:
        ...
        courses_data = await self.db[self.collection_name].find({"instructor_id": instructor_id}).to_list(length=100)
        return [Course(id=str(course.pop("_id")), **course) for course in courses_data]
    ...
    async def search(self, query: str, skip: int = 0, limit: int = 100) -> List[Course]:
        ...
        courses_data = await self.db[self.collection_name].find(search_query).skip(skip).limit(limit).to_list(length=limit)
        return [Course(id=str(course.pop("_id")), **course) for course in courses_data]
    ...
    async def delete(self, course_id: str) -> bool:
        """Eliminar curso"""
        result = await self.db[self.collection_name].delete_one({"_id": ObjectId(course_id)})
