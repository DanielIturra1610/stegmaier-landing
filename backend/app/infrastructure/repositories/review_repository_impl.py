...
class MongoDBReviewRepository(ReviewRepository):
    ...
    async def create(self, review: Review) -> Review:
        ...
        result = await self.db[self.collection_name].insert_one(review_dict)
        review.id = str(result.inserted_id)
        return review
    
    async def get_by_id(self, review_id: str) -> Optional[Review]:
        """Obtener reseña por ID"""
        review_data = await self.db[self.collection_name].find_one({"_id": ObjectId(review_id)})
        if review_data:
    ...
    async def get_by_user_and_course(self, user_id: str, course_id: str) -> Optional[Review]:
        ...
        review_data = await self.db[self.collection_name].find_one(
    ...
    async def get_by_course(self, course_id: str, skip: int = 0, limit: int = 100) -> List[Review]:
        ...
        reviews_data = await self.db[self.collection_name].find(
            {"course_id": course_id}
    ...
    async def delete(self, review_id: str) -> bool:
        """Eliminar reseña"""
        result = await self.db[self.collection_name].delete_one({"_id": ObjectId(review_id)})
    ...
    async def calculate_course_average(self, course_id: str) -> float:
        ...
        result = await self.db[self.collection_name].aggregate(pipeline).to_list(length=1)
        if result:
