...
class MongoDBLessonRepository(LessonRepository):
    ...
    async def create(self, lesson: Lesson) -> Lesson:
        ...
        result = await db[self.collection_name].insert_one(lesson_dict)
    ...
    async def get_by_id(self, lesson_id: str) -> Optional[Lesson]:
        ...
        db = await get_database()
        lesson_data = await db[self.collection_name].find_one({"_id": ObjectId(lesson_id)})
        if lesson_data:
    ...
    async def get_by_course(self, course_id: str) -> List[Lesson]:
        ...
        db = await get_database()
        lesson_data = await db[self.collection_name].find(
            {"course_id": course_id}
    ...
    async def list(self, skip: int = 0, limit: int = 100) -> List[Lesson]:
        ...
        db = await get_database()
        lessons_data = await db[self.collection_name].find().skip(skip).limit(limit).to_list(length=limit)
    ...
    async def delete(self, lesson_id: str) -> bool:
        """Eliminar lección"""
        db = await get_database()
        result = await db[self.collection_name].delete_one({"_id": ObjectId(lesson_id)})
    ...
    async def reorder(self, course_id: str, lesson_order: List[dict]) -> bool:
        ...
        if operations:
            result = await db[self.collection_name].bulk_write(operations)
