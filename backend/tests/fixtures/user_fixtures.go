...
// AsSuperAdmin sets the user as a superadmin
...
// SuperAdminUser returns a superadmin user fixture
func SuperAdminUser(tenantID string) *UserFixture {
	return NewUserBuilder(tenantID).
		AsSuperAdmin().
		WithEmail("superadmin@example.com").
		WithFullName("Test SuperAdmin").
		Verified().
		Build()
}
...
func MultipleUsers(tenantID string, count int) []*UserFixture {
	...
	roles := []string{"student", "instructor", "admin", "superadmin"}
